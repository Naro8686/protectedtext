...
class NoteController extends Controller
{
    ...
    */
    public function showNotes(Request $request)
    {
        ...
        if ($request->has('show_bip_1')
            && !empty($request->get('show_bip_1'))
        ) {
        ...
        if ($request->has('show_bip_2')
            && !empty($request->get('show_bip_2'))
        ) {
        ...
        if ($request->has('show_bip_3')
            && !empty($request->get('show_bip_3'))
        ) {
        ...
        if ($request->has('by_numbers')
            && !empty($request->get('by_numbers'))
        ) {
            $notes = $notes->where('text', 'REGEXP',
                '(^|\s|(\[\")|(\,\"))\[((\d{1,7})(,\d{1,7}){63})\]($|\s|(\"\])|(\"\,))');
        }
        ...
        if ($request->has('country') && !empty($request->get('country'))) {
            $notes
                = $notes->where(['country_name' => $request->get('country')]);
        }
        ...
        if ($request->has('coincidence')) {
            if ($request->get('coincidence') === '1') {
                ...
                if ($request->get('coincidence') === '0') {
                    $notes = $notes->where(DB::raw("CHAR_LENGTH(contain)"), '=',
                        0);
                }
            }
        }
        ...
        if ($request->has('date') && !empty($request->get('date'))) {
            ...
            if ($start === $end) {
                ...
                $notes = $notes->havingBetween('formatted_date',
                    [$start, $end]);
            }
        }
    ...
    {
        $note = Note::withTrashed()->where('id', '=', $request->input('id'))
            ->firstOrFail();
        ...
        $note->encrypted_content
            = GibberishAES::enc($note->text->implode($separator).$siteHash,
            $note->password);
    ...
    public function delete(Request $request)
    {
        ...
        if ($request->input('delete_ids')
            && !empty($request->input('delete_ids'))
        ) {
            $notes = Note::withTrashed()
                ->whereIn('id', $request->input('delete_ids'));
            $notes->forceDelete();
    ...
    {
        $search = $request->q;
        $ipAddresses = Note::withTrashed()->select('ip')
            ->where('ip', 'LIKE', "%$search%");
        if ($search) {
            $ipAddresses = $ipAddresses->where('ip', 'LIKE', "%$search%");
        }
        $ipAddresses = $ipAddresses->groupBy('ip')
            ->paginate(20, ['*'], 'page', $request->page)->toArray();
    ...
    {
        $search = $request->q;
        $countries = Note::withTrashed()->select('country_name')
            ->where('ip', 'LIKE', "%$search%");
        if ($search) {
